...
foreach($hashes as $hash){
	...
	$usertalkmap[$user]  = $json["talk_ids"];
	$pophashes = array_merge($pophashes, $usertalkmap[$user]);
}
...
foreach($pophashes as $k=>$v){
	foreach($talks_array as $talk){
		if($k == $talk["event_id"]){
			...
			foreach($usertalkmap as $user=> $values){
			...
			echo $v, "(", implode(",", $agreeing), ") : <i>", $talk["track_name"], "</i> -- ", $talk["title"], "\n";
			echo "</div>";
			continue;
		}
	}
